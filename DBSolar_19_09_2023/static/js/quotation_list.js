...
(function(){
  ...
  function applyFilters(searchInput, filterType, filterProject, quickFilter) {
    ...
    document.querySelectorAll('#quotationTableBody tr').forEach(r => {
      ...
      const isLatest = (r.dataset.latest || '0') === '1';
      const quotationNo = (r.dataset.quotationNo || '').toString();
      const isConfirmed = (r.dataset.confirmed || '0') === '1';
      const isConverted = (r.dataset.converted || '0') === '1';
      const isRevise = (r.dataset.revise || '0') === '1';
      const isRevisionNo = quotationNo.indexOf('_') !== -1;

      let okQuick = true;
      switch ((quickFilter || 'all').toLowerCase()) {
        case 'new':
          // New: pending + latest + quotation number without underscore revisions
          okQuick = !isConfirmed && isLatest && !isRevisionNo;
          break;
        case 'revise':
          // Revise: quotation numbers with "_" and pending action set (Edit/Revise/Confirm/PDF)
          okQuick = isLatest && !isConfirmed && isRevisionNo;
          break;
        case 'confirm':
          // Confirm: confirmed rows where action has Convert Consumer
          okQuick = isLatest && isConfirmed && !isConverted;
          break;
        case 'convert':
          // Confirmed but not yet converted (shows Convert Consumer button)
          okQuick = isLatest && isConfirmed && !isConverted;
          break;
        case 'converted':
          // Already converted quotations (shows Converted label)
          okQuick = isLatest && isConverted;
          break;
        default:
          // All: show latest rows; older rows are visible through Details
          okQuick = isLatest;
      }

      r.style.display = (okText && okType && okProject && okQuick) ? '' : 'none';
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    const quickFilterWrap = document.getElementById('quotationQuickFilter');
    const quickFilterButtons = quickFilterWrap ? quickFilterWrap.querySelectorAll('.quotation-segment') : [];
    let currentQuickFilter = 'all';

    if (applyBtn) applyBtn.addEventListener('click', () => applyFilters(searchInput, filterType, filterProject, currentQuickFilter));
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', function(){
        ...
        quickFilterButtons.forEach(btn => {
          const isAll = (btn.dataset.filter || 'all') === 'all';
        ...
        applyFilters(searchInput, filterType, filterProject, currentQuickFilter);
      });
    }
    if (searchInput) searchInput.addEventListener('input', () => applyFilters(searchInput, filterType, filterProject, currentQuickFilter));
    if (filterType) filterType.addEventListener('change', () => applyFilters(searchInput, filterType, filterProject, currentQuickFilter));
    if (filterProject) filterProject.addEventListener('change', () => applyFilters(searchInput, filterType, filterProject, currentQuickFilter));

    quickFilterButtons.forEach(btn => {
      btn.addEventListener('click', function() {
        currentQuickFilter = (this.dataset.filter || 'all').toLowerCase();
        quickFilterButtons.forEach(x => {
          const isActive = x === this;
    ...
    if (exportBtn) exportBtn.addEventListener('click', function(e){
      ...
      document.querySelectorAll('#quotationTableBody tr').forEach(tr => {
        ...
        rows.push([
          ...
          cols[2]?.innerText.trim(), // Associate
          cols[3]?.innerText.trim(), // Type
          cols[4]?.innerText.trim(), // Project
          cols[5]?.innerText.trim(), // Capacity
          cols[6]?.innerText.trim(), // Date
          cols[7]?.innerText.trim(), // Final Amount
          cols[8]?.innerText.trim()  // Status
        ]);
      });
