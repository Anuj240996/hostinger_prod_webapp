...
def index(request):
    ...
    profile = Profile.objects.all()
    totalfire = Firereport.objects.all().count()
    totalAdmin = User.objects.filter(is_staff=True, is_active=True).count()
    notification1 = staff_Notification.objects.filter(staff_id=request.user.id, status=False).order_by('-created_at')
    totalStockist = Customer.objects.all().count()
    count1 = staff_Notification.objects.filter(staff_id=request.user.id, status=False).count()
    ...
    #
    # # Count the distinct comp_name entries based on the installation_date_date criterion
    # total_warranty_quantity = MSEB.objects.filter(installation_date_date__isnull=False).values(
    #     'comp_name').distinct().count()

    # Calculate the date 30 days ago from today in the current timezone
    now = dj_timezone.now()
    last_30_days = now - timedelta(days=30)


    # Count the distinct comp_name entries based on the installation_date_date criterion
    total_warranty_quantity = MSEB.objects.filter(
        installation_date_date__isnull=False,  # Exclude null installation dates
        ).values('comp_name').distinct().count()
...
def customers(request):
    ...
    #     template_path = 'dashboard/customers_pdf_template.html'  # Create a separate template for PDF content
    #     context_for_pdf = context  # Pass the same context data to the PDF template
    #     html_content = render_to_string(template_path, context_for_pdf)
    #     pdf = BytesIO()
